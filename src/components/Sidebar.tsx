...
import { Search, MoreVertical, MessageSquarePlus, Users, Radio, LogOut, Sun, Moon, Plus, UserPlus } from 'lucide-react';
    ...
    <div className={`w-full md:max-w-[380px] border-r border-zinc-200 dark:border-zinc-800 flex flex-col h-full bg-white dark:bg-zinc-900 z-20 ${className}`}>
      ...
      <div className="p-4 flex items-center justify-between bg-zinc-50 dark:bg-zinc-800/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold dark:text-white leading-tight">{profile?.displayName}</h3>
            <span className="text-[10px] text-green-500 font-bold uppercase tracking-widest">Active</span>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={onDarkModeToggle} className="p-2 rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">
            ...
            <AnimatePresence>
              {showAddMenu && (
                <motion.div 
                  ...
                  className="absolute top-12 right-0 w-52 bg-white dark:bg-zinc-800 shadow-2xl rounded-2xl p-2 border border-zinc-100 dark:border-zinc-700 z-50 overflow-hidden"
                >
                  <button onClick={() => {
                    const name = prompt('Guruh nomini kiriting:');
                    if (name) onCreateRoom('group', name);
                    setShowAddMenu(false);
                  }} className="w-full flex items-center gap-3 p-3 hover:bg-zinc-50 dark:hover:bg-zinc-700/50 rounded-xl text-sm font-medium text-zinc-700 dark:text-zinc-200 transition-colors">
                  ...
                  <button onClick={handleLogout} className="w-full flex items-center gap-3 p-3 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl text-sm font-medium text-red-600 dark:text-red-400 transition-colors">
      ...
      <div className="px-4 py-3">
        <div className="relative group">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400 group-focus-within:text-green-500 transition-colors" />
          <input 
            type="text" 
            placeholder="Ism yoki username..."
            className="w-full pl-10 pr-4 py-2.5 bg-zinc-100 dark:bg-zinc-800 rounded-2xl outline-none border border-transparent focus:border-green-500/50 focus:bg-white dark:focus:bg-zinc-900 transition-all text-zinc-900 dark:text-white text-sm"
            value={search}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        {foundUsers.length > 0 && (
          <div className="mb-4">
            <div className="px-5 py-2 text-[10px] uppercase font-bold text-green-600 dark:text-green-400 tracking-widest bg-green-50/50 dark:bg-green-900/10">
              {search ? 'Qidiruv natijalari' : 'Global kontaktlar'}
            </div>
            {foundUsers.map(user => (
              <div 
                key={user.uid}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-bold dark:text-white truncate">{user.displayName}</h4>
                  <p className="text-[10px] text-green-500 font-bold">@{user.username}</p>
                </div>
        ...
        <div>
          ...
          {filteredRooms.map(room => (
            <div 
              ...
              className={`flex items-center gap-4 px-5 py-3.5 cursor-pointer transition-all ${activeRoomId === room.id ? 'bg-green-50/80 dark:bg-green-900/10 border-l-4 border-green-500 px-[16px!important]' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50 border-l-4 border-transparent'}`}
            >
              <div className="w-12 h-12 rounded-full flex items-center justify-center overflow-hidden bg-zinc-100 dark:bg-zinc-800 shadow-sm relative">
                <img 
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate flex items-center gap-1.5 font-medium">
                  {room.type === 'channel' && <Radio className="w-3 h-3 text-purple-400" />}
                  {room.type === 'group' && <Users className="w-3 h-3 text-blue-400" />}
                  {room.lastMessage}
                </p>
              </div>
            </div>
          ...
          {filteredRooms.length === 0 && foundUsers.length === 0 && (
            <div className="p-10 text-center">
              ...
              <p className="text-zinc-400 text-sm font-medium">Chatlar topilmadi</p>
            </div>
          )}
        </div>
      </div>
